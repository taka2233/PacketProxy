...
public class GUIProjectChooserDialog {
	...
	private boolean setupNewProject(Component parent) throws Exception {
		...
		gbc.weightx = 0.0;
		panel.add(new JLabel(I18nString.get("Enter project name") + ":"), gbc);
		...
		gbc.weightx = 0.0;
		gbc.gridwidth = 2;
		panel.add(new JLabel(I18nString.get("Save directory") + ":"), gbc);

		var defaultDir = Paths.get(System.getProperty("user.home"), ".packetproxy", "projects");
		var locationField = new JTextField(defaultDir.toString(), 30);
		locationField.setEditable(false);

		var browseButton = new JButton(I18nString.get("Browse..."));
		browseButton.addActionListener(e -> {
			var chooser = new NativeFileChooser(locationField.getText());
			chooser.setDialogTitle(I18nString.get("Select save directory"));
			if (chooser.showDirectoryDialog(parent) == NativeFileChooser.APPROVE_OPTION) {
		...
		// 名前が空の場合はOKボタンを無効化
		var okButton = Arrays.stream(optionPane.getComponents()).filter(c -> c instanceof JPanel)
				.flatMap(c -> Arrays.stream(((JPanel) c).getComponents())).filter(b -> b instanceof JButton)
				.map(b -> (JButton) b)
				.filter(b -> b.getText() != null && (b.getText().equals("OK")
						|| b.getText().equals(UIManager.getString("OptionPane.okButtonText"))))
		...
		if (okButton != null) {
			...
			nameField.getDocument().addDocumentListener(new DocumentListener() {
				...
				private void update() {
					finalOkButton.setEnabled(!nameField.getText().trim().isEmpty());
				}
			});
		}
		...
		projects.createNewProject(name, saveDir);
		return true;
	}
