...
public class GUIAutoColorRuleAddDialog extends JDialog {
	...
	private JComponent buttons() {
		...
		panel_button.setMaximumSize(new Dimension(Short.MAX_VALUE, button_add.getMaximumSize().height));
	...
	public GUIAutoColorRuleAddDialog(JFrame owner) throws Exception {
		this(owner, "", null);
	}

	public GUIAutoColorRuleAddDialog(JFrame owner, String defaultPattern) throws Exception {
		this(owner, defaultPattern, null);
	}

	public GUIAutoColorRuleAddDialog(JFrame owner, String defaultPattern, AutoColorRule.Color defaultColor) throws Exception {
		super(owner);
		setTitle("Add Auto Color Rule");
		...
		text_pattern.setText(defaultPattern);

		// デフォルトカラーが指定されている場合は設定
		if (defaultColor != null) {
			combo_color.setSelectedItem(defaultColor.getValue());
		}
		...
		button_add.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String colorValue = (String) combo_color.getSelectedItem();
					AutoColorRule.Color color = AutoColorRule.Color.fromValue(colorValue);
					AutoColorRule rule = new AutoColorRule(pattern, color);
					AutoColorRules.getInstance().create(rule);
					dispose();
				} catch (Exception e1) {
					errWithStackTrace(e1);
					JOptionPane.showMessageDialog(owner, "Failed to add rule: " + e1.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	}
