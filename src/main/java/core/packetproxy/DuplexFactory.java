...
public class DuplexFactory {
	...
	public static DuplexSync createDuplexSync(Endpoint client_endpoint, Endpoint server_endpoint, String encoder_name,
			String ALPN) throws Exception {
		DuplexSync duplex = new DuplexSync(server_endpoint);
		prepareDuplex(duplex, client_endpoint, server_endpoint, encoder_name, ALPN);
		return duplex;
	}

	public static DuplexAsync createDuplexAsync(Endpoint client_endpoint, Endpoint server_endpoint, String encoder_name)
			throws Exception {
	...
	private static void prepareDuplex(final Duplex duplex, Endpoint client_endpoint, Endpoint server_endpoint,
			final String encoder_name, String ALPN) throws Exception {
	...
	public static DuplexSync createDuplexSyncFromOneShotPacket(final OneShotPacket oneshot) throws Exception {
		...
		duplex.addDuplexEventListener(new Duplex.DuplexEventListener() {
			...
			public byte[] onClientChunkSend(byte[] data) throws Exception {
				...
				if (data.length > TOO_LARGE_LENGTH) {
					...
					byte[] omitData = String
							.format("*** Data cannot be displayed (reason: Data too large: %d) ***", data.length)
